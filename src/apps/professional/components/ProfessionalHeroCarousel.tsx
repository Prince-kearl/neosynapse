...
const slides: ProfessionalSlide[] = [
  {
    id: "1",
    title: "Telemedicine",
    subtitle: "Speak with patients through secure video consultation.",
    image: heroTele,
    buttonLabel: "Start Consultation",
    buttonLink: "/professional/telemedicine",
    icon: Video,
  },
  {
    id: "2",
    title: "Patient Management",
    subtitle: "Access assigned patients and review their latest details.",
    image: heroSymptom,
    buttonLabel: "View Patients",
    buttonLink: "/professional/patients",
    icon: Users,
  },
  {
    id: "3",
    title: "Clinical Notes",
    subtitle: "Review, revise, and finalize encounter notes quickly.",
    image: heroAI,
    buttonLabel: "Open Notes",
    buttonLink: "/professional/notes",
    icon: FileText,
  },
];
...
export function ProfessionalHeroCarousel({ displayName = "Doctor" }: ProfessionalHeroCarouselProps) {
  const [api, setApi] = useState<CarouselApi | null>(null);
  const [current, setCurrent] = useState(0);
  const count = slides.length;
  const firstName = displayName.trim().split(/\s+/)[0] || "Doctor";
  ...
  return (
    <section className="space-y-4 max-[640px]:space-y-3 max-[480px]:space-y-2.5">
        ...
        <CarouselContent className="-ml-2 md:-ml-4 max-[820px]:ml-0">
          {slides.map((slide) => (
            <CarouselItem key={slide.id} className="pl-2 md:pl-4 max-[820px]:pl-0">
              <div className="relative flex h-[180px] w-full items-stretch overflow-hidden rounded-2xl bg-gradient-to-br from-[#63C88F] to-[#53B67B] p-0 sm:h-[200px] md:h-[190px] max-[820px]:h-[460px] max-[820px]:rounded-[30px] max-[640px]:h-[210px] max-[520px]:h-[178px] max-[480px]:h-[156px]">
      ...
      <div className="mt-2 flex justify-center gap-2 max-[640px]:mt-1.5 max-[640px]:gap-1.5">
