...
function buildSymptomReport(params: {
  ...
  return {
    markdown: report.clinicalMarkdown,
    patientMarkdown: report.patientMarkdown,
    json: {
      ...report.json,
...
export default function PatientSymptomChecker() {
  ...
  useEffect(() => {
    ...
    const autoReport = buildSymptomReport({
      result,
      userId: user.id,
      patientName: assessmentFor === "other"
        ? normalizedName
        : String(user.user_metadata?.full_name || user.user_metadata?.display_name || user.user_metadata?.name || user.email || ""),
