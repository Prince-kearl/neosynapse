...
const symptomCheckerCopy = {
  en: {
    ...
    possibleConditionsIntro: "These are possible explanations for your symptoms generated by the AI triage engine. They are not diagnoses.",
    possibleConditionsHint: "Each condition includes a concise medical definition, unique symptom-specific reasoning, likely causes, supporting symptoms, first aid, and treatment context. Overall severity is separate from individual condition likelihood.",
      ...
      <div className="flex-1 min-h-screen bg-background">
        <div className="p-4 lg:p-6 max-w-3xl mx-auto space-y-6">
          ...
          <div className={`rounded-2xl p-6 border ${config.color}`}>
            ...
            <p className="text-sm opacity-90 mt-3">{result.summary}</p>
          ...
          <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-[0_6px_18px_rgba(15,23,42,0.06)]">
            ...
            <div className="space-y-4">
              {result.possible_conditions.map((c) => (
                <div key={c.name} className="rounded-2xl border border-border/60 p-4 bg-background shadow-sm">
                  ...
                  <p className="mt-3 text-sm text-foreground"><span className="font-medium">Definition:</span> {c.definition ?? "A concise medical definition is not available for this condition yet."}</p>
                  <p className="mt-3 text-sm text-muted-foreground"><span className="font-medium">Reason:</span> {c.reason}</p>
                  {c.causes && (
                    <p className="mt-2 text-sm text-muted-foreground"><span className="font-medium">Causes:</span> {c.causes}</p>
                  )}
                  {c.symptoms && (
                    <p className="mt-2 text-sm text-muted-foreground"><span className="font-medium">Symptoms:</span> {c.symptoms}</p>
                  )}
                  {c.first_aid && (
                    <div className="mt-3 rounded-xl border border-primary/15 bg-primary/5 p-3">
                      <p className="text-sm text-foreground">
                        <span className="font-medium">First aid now:</span> {c.first_aid}
                      </p>
                    </div>
                  )}
                  {c.treatments && (
                    <p className="mt-2 text-sm text-muted-foreground"><span className="font-medium">Treatment:</span> {c.treatments}</p>
                  )}
                  {Array.isArray(c.sources) && c.sources.length > 0 && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      <span className="font-medium">Sources:</span> {c.sources.join(", ")}
                    </p>
                  )}
                  <p className="mt-2 text-xs text-muted-foreground">{likelihoodHelp[c.likelihood]}</p>
                </div>
              ))}
            </div>
          </div>
